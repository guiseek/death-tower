...
export function drawWinner(config: Config, code?: number) {
  ...
  if (config.ctx && config.canvas) {
    ...
    config.ctx.fillText(config.state.winner.text, 600, 440 - easing(config.state.winner.opacity / 100) * 40)
    
    if (code) {
      config.ctx.fillStyle = 'rgba(255, 255, 255, ' + config.state.winner.opacity / 100 + ')'
      config.ctx.font = "48px 'Germania One', cursive"
      config.ctx.fillText(`Desafio: #${code}`, 650, 520 - easing(config.state.winner.opacity / 100) * 40)
    }
  }
