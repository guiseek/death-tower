...
export function drawSky(config: Config) {
  if (config.storage.sky == null) {
    ...
    for (let i = 0; i < 150; i++) {
      const starSize = Math.floor(Math.random() * config.sky.starSizes.length)
    ...
    config.ctx!.drawImage(config.storage.sky, skypos - config.canvas!.width, skyYPos)
    config.ctx!.drawImage(config.storage.sky, skypos, skyYPos - config.canvas!.height)
