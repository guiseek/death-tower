...
class point {
  ...
  addVector(v: vector) {
    ...
    const res = new point(this.x + v.x, this.y + v.y);
    return res;
  }
...
class vector {
  ...
  sum(other: { x: number; y: number; }) {
    /* return the sum of this and another vector v */
    const res = new vector(this.x + other.x, this.y + other.y);
    return res;
  }

  product(lambda: number) {
    /* return the external product of this per lambda*/
    const res = new vector(lambda * this.x, lambda * this.y);
    return res;
  }

  scalarProduct(other: vector) {
  ...
  orthogonalVector() {
    ...
    const res = new vector(-this.y, this.x);
    return res;
  }
...
class straightLine {
  // point1: point;
  point2: point;
  constructor(public point1: point, public element: any) {
  ...
  containPoint(point: { x: number; y: number; }) {
    const equationLine = this.equation();
    return (
      keepNDec(
...
class segment {
  ...
  containPoint(point: any) {
    ...
    const scalarProd1 = vector1.scalarProduct(vector2);
    const scalarProd2 = vector1.scalarProduct(vector1);

    return scalarProd1 >= 0 && scalarProd1 <= scalarProd2;
  }

  intersect(other: { point1: any; point2: any; }) {
    /*  other is antoher segment of the line with direction this. Return true if intersections between
        this and other is not null*/

    const segmentVector = new vector(this.point1, this.point2);
    const vector1 = new vector(this.point1, other.point1);
    const vector2 = new vector(this.point1, other.point2);
    const vector3 = new vector(this.point2, other.point1);
    const vector4 = new vector(this.point2, other.point2);

    const scalarProd1 = segmentVector.scalarProduct(vector1);
    const scalarProd2 = segmentVector.scalarProduct(vector2);
    const scalarProd3 = segmentVector.scalarProduct(vector3);
    const scalarProd4 = segmentVector.scalarProduct(vector4);
...
class polygon {
  ...
  copy() {
    ...
    this.vertices.forEach((point: { copy: () => any; }) => {
  ...
  edges() {
    ...
    if (nbVertices > 2) {
      ...
      const edge = new segment(this.vertices[0], this.vertices[1]);
      edges.push(edge);
    }
    return edges;
  }
  ...
  isoBarycenter() {
    ...
    this.vertices.forEach((point: { x: number; y: number; }) => {
  ...
  // static separation(other: polygon, edge: segment, barycenter: point) {
  static separation(other: polygon, edge: segment, barycenter: point) {

    const otherNbVertices = other.vertices.length;
    const segmentLine = new straightLine(edge.point1, edge.point2);
    const equation = segmentLine.equation();

    const thisSide =
      equation[0] * barycenter.x + equation[1] * barycenter.y + equation[2];

    const pointSideSet = [];
    let pointSide: point[] | number = [];

    const pointOnSepartor = [];
    ...
    if (pointOnSepartor.length == 1) {
      ...
      const alignSegment = new segment(pointOnSepartor[0], pointOnSepartor[1]);
    ...
    if (commonPoint) {
      ...
      const minPointSide = Math.min(...pointSideSet);
      const maxPointSide = Math.max(...pointSideSet);
  ...
  sat(other: polygon) {
    ...
    const thisEdges = this.edges();
    const otherEdges = other.edges();

    const thisBarycenter = this.isoBarycenter();
    const otherBarycenter = other.isoBarycenter();
...
class square extends polygon {
  center: any[] | point;
  polarDirection: number[] | undefined;
  ...
  constructor(element1: polygon, element2: number[] | undefined) {
    ...
    if (element2 instanceof point) {
      ...
      diagonal = new segment(point1, point3);
      const initialCenter = diagonal.center();
      center = element1;
      const translationVector = new vector(initialCenter, center);
  ...
  rotate(angle: any) {
    ...
    const diagonal = new segment(this.vertices[0], this.vertices[2]);
    const initialCenter = diagonal.center();
    const translationVector = new vector(initialCenter, this.center);
  ...
  translate(transactionVector: any) {
...
class hero {
  body: square;
  foot: polygon[];
  vx: number | boolean | number[];
  vy0: number | boolean | number[];
  xJump: number | boolean | number[];
  yJump: number | boolean | number[];
  g: number | boolean | number[];
  t: number | boolean | number[];
  isJumping: number | boolean | number[];
  startJumpPosition: any;
  hasStarted: boolean;
  isDead: boolean;
  ...
  havefinished: boolean;
  constructor(
    positionCenter: number | boolean | number[],
    positionPolarCoordinates: string | number | boolean | any[],
    vx: number | boolean | number[],
    vy0: number | boolean | number[],
    xJump: number | boolean | number[],
    yJump: number | boolean | number[],
    ...
    /*  Body hitBox */
    const intialPosition = new point(positionCenter[0], positionCenter[1]);
    this.body = new square(intialPosition, positionPolarCoordinates.slice());

    /*  foot hitBox */
    const footPoint = this.body.getLowestPointIndex();
    if (footPoint.length == 2) {
      // In that case, the first point of the foot if the leftest because of mthode getLowestPointIndex
      const footPoint1 = this.body.vertices[footPoint[0]].copy();
      const footPoint2 = this.body.vertices[footPoint[1]].copy();
      const foot1 = new polygon([footPoint1, footPoint2]);
      this.foot = [foot1];
    } else {
      ...
      const footPoint3 =
        this.body.vertices[positivMod(footPoint[0] - 1, 4)].copy();
      const foot1 = new polygon([footPoint1, footPoint2]);
      const foot2 = new polygon([footPoint1.copy(), footPoint3]);
  ...
  translate(transactionVector: vector) {
    /* translate the hero : body + foot */
    this.body.translate(transactionVector);
    this.foot.forEach((footValue: { translate: (arg0: any) => void; }) => {
      footValue.translate(transactionVector);
    });
  }

  footContactWithRoof(previousFoot: any[], platformInstance: platform) {
    ...
    for (let k = 0; k < this.foot.length; k++) {
      const lineTest = new straightLine(
  ...
  move(gridInstance: { grid: string | any[]; }) {
    ...
    const previousFoot: any[][] = [];
    this.foot.forEach((foot: { vertices: { copy: () => any; }[]; }) => {
    ...
    const tSauv = this.t;
    const yPosSauv = this.body.center.y;
    const xPosSauv = this.body.center.x;
    // use to rectify position when landing

    const dt = frameTimeDiff.dt;
    ...
    */
    ...
    const deadContactElementCenter: any[] = [];
    ...
    aroundGrid.forEach((col: any[] | undefined) => {
      // one test contact for each potential elements
      if (col != undefined) {
        // if columns grid is empty
        col.forEach((element: { platform: { center: { y: any; }; }; peak: any; center: { y: any; }; ending: any; }) => {
    ...
    if (this.body.center.y < 0) {
      ...
      this.isDead = true;
      const newCenter = new point(this.body.center.x, 0.5);
      const translateVector = new vector(this.body.center, newCenter);
    ...
    if (!this.isDead) {
      if (floorContactElementCenter.length > 0) {
        ...
        if (this.isJumping) {
          ...
          const c = -(maxFloorContactCenter + 1 - yPosSauv);
        ...
        const newCenter = new point(newXPosition, maxFloorContactCenter + 1);
        const translateVector = new vector(this.body.center, newCenter);
        this.translate(translateVector);
...
class platform {
  ...
  */
  constructor(x: number, y: number) {
    ...
    const platformCenter = new point(x + 1 / 2, y + 1 / 2);
...
class peak {
  ...
  /*  peak is a triangle. There is 4 kind of peak according to there orientation.
      ...
      of the hero to test collisions
  */
  constructor(x: number, y: number, orientation: any) {
...
class ending {
  ...
  */
  constructor(position: number) {
    ...
    const point1 = new point(position, 0);
    const point2 = new point(position + 1, 0);
    const point3 = new point(position + 1, 10);
    const point4 = new point(position, 10);
...
class checkPoint {
  ...
  */
  ...
  update(heroInstance: { body: { center: { x: any; y: any; }; }; }) {
    this.x = heroInstance.body.center.x;
    this.y = heroInstance.body.center.y;
    this.col = Math.floor(this.x);
  }
...
class grid {
  ...
  addPlatform(x: number, y: number) {
    const platformInstance = new platform(x, y);
  ...
  addPeak(x: any, y: any, direction: any) {
    const peakInstance = new peak(x, y, direction);
  ...
  addEnding(endingInstance: { col: string | number; }) {
  ...
  addCheckPoint(checkPointInstance: { col: string | number; }) {
  ...
  removeCol(start: any, end: number) {
...
class drawing {
  ...
  height: number;
  unity: number;
  heroCenterXPosition: number;
  heroAjustYPosition: number;
  deathAnimationTime: number;
  winAnimationTime: number;
  ctxBack: any;
  backGroundTimeScroll: number;
  backgroundSpeed: number;
  backgroundImageCity: HTMLElement | null;
  backgroundImageCityReverse: HTMLElement | null;
  sound: { backGroundMusic: HTMLElement | null; checkpointSaveTime: number; deathSound: HTMLElement | null; };
  ...
  constructor(heroInstance: hero) {
    ...
    // background canvas
    const canvasBack = document.getElementById('canvas-background');
  ...
  setGridPosition(heroInstance: { body: { center: { x: number; y: number; }; }; }) {
  ...
  gridAbscissa(x: number) {
    /*  scale abscissa to this.unity */
    return x * this.unity;
  }

  gridOrdinate(y: number) {
  ...
  drawSquareNeonStyle(elementToDraw: Path2D, r: string | number, g: string | number, b: string | number) {
  ...
  drawHero(heroInstance: { body: { vertices: { y: any; }[]; }; }) {
    /* draw hero and print it on screen */
    const heroBody = new Path2D();
    heroBody.moveTo(
      this.gridAbscissa(
  ...
  drawGrid(gridInstance: { grid: any[]; }, heroInstance: { body: { center: { x: number; }; }; isDead: any; haveFinished: any; }) {
    /*  draw all possible of a grid */
    const minColToKeep = Math.floor(Math.max(this.heroCenterXPosition, 0));
    const maxColToKeep = Math.floor(heroInstance.body.center.x + 40);

    const platformDraw = new Path2D();
    const peakDraw = new Path2D();
    const checkPointDraw = new Path2D();
    const endingDraw = new Path2D();
    let endingPosition;

    gridInstance.grid.slice(minColToKeep, maxColToKeep).forEach((gridRow: any[] | undefined) => {
      ...
      if (gridRow != undefined) {
        gridRow.forEach((element: { platform: { vertices: { y: any; }[]; }; peak: { vertices: { y: any; }[]; }; x: number; y: number; col: number; }) => {
          if (element instanceof platform) {
            platformDraw.rect(
              this.gridAbscissa(
  ...
  deathAnimation(heroInstance: { deathParticle: any[]; }) {
    //  Draw particles when death
    let translationVector;
    const explosion = new Path2D();

    heroInstance.deathParticle.forEach((particle: { angle: number; maxProjection: number; position: { translate: (arg0: vector) => void; x: number; y: any; }; }) => {
  ...
  drawBackGround() {
    ...
    const imageHeight = this.ctxBack.canvas.height;
    ...
    const xPosition = Math.floor(this.backGroundTimeScroll / imageWidth);
...
function restart(parameters: any[]) {
...
function keyEventHandler(event: { code: string | number; type: string; preventDefault: () => void; }) {
...
function platformDistance(h: number, heroInstance: { yJump: number; xJump: number; vx: number; }) {
...
function level1(gridInstance: grid, heroInstance: hero) {
  gridInstance.grid = [];

  const d1 = platformDistance(1, heroInstance);
  const d2 = platformDistance(2, heroInstance);
  const d27 = platformDistance(2.7, heroInstance);
  const d0 = 4;
  const dMoins1 = platformDistance(-1, heroInstance);
  ...
  const endingInstance = new ending(Math.floor(lastPos[0]) + 33);
