import { Config, Point } from '../../types/config';
import { getCirclePoint } from './math/get-circle-point';
...
export function brickFactory(config: Config, height: number, pos: number) {
  ...
  const gradient = temp.ctx.createLinearGradient(
    0,
    0,
    temp.canvas.width,
    height
